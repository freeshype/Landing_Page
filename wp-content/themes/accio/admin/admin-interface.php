...
function siteoptions_add_admin() {
    ...
    $ac_page = add_theme_page('Theme Options', 'Theme Options', 'edit_theme_options', 'siteoptions','siteoptions_options_page');
	add_action("admin_print_scripts-$ac_page", 'of_load_only');
	add_action("admin_print_styles-$ac_page",'of_style_only');
}
...
<div class="wrap" id="accio_container">
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      <div class="logo">
        <h2>Accio <span class="accio_version">v1.0</span></h2>
      </div>
...
<script type="text/javascript">
			...
			jQuery('#expand_options').click(function(){
				if(flip == 0){
					flip = 1;
					jQuery('#accio_container #of-nav').hide();
					jQuery('#accio_container #content').width(755);
					jQuery('#accio_container .group').add('#accio_container .group h2').show();

					jQuery(this).text('[-]');

				} else {
					flip = 0;
					jQuery('#accio_container #of-nav').show();
					jQuery('#accio_container #content').width(579);
					jQuery('#accio_container .group').add('#accio_container .group h2').hide();
					jQuery('#accio_container .group:first').show();
					jQuery('#accio_container #of-nav li').removeClass('current');
					jQuery('#accio_container #of-nav li:first').addClass('current');

...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="hentry-box">
		...
		$quote_text = rwmb_meta('accio_quote_text', 'type=textarea');
		$quote_author = rwmb_meta('accio_quote_author', 'type=text');
		$image_id = get_post_thumbnail_id();
		$image_url = wp_get_attachment_image_src($image_id, true);
		$color = rwmb_meta('accio_quote_bg', 'type=color');
		$opacity = rwmb_meta('accio_quote_bg_opacity', 'type=slider');
		$opacity = $opacity / 100;
		...
		<?php $show_quote_meta = rwmb_meta('accio_show_quote_meta', 'type=select'); ?>
